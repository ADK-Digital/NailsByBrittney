...
function DraggableList({ items, renderFields }) { return <ul>{items.map((item, idx) => <li key={item.id} className="admin-item">{renderFields(item, idx)}</li>)}</ul>; }
...
function AppointmentCard({ appointment, onRefresh }) {
  ...
  return <article className="card appointment-card">
    <div className="appointment-head">
      ...
      <span className="pill">service: {appointment.service_payment_status || 'unpaid'}</span>
      <span className="pill">late: {appointment.late_fee_status || 'unpaid'}</span>
      <span className="pill">no-show: {appointment.no_show_fee_status || 'unpaid'}</span>
    </div>
    ...
    <div className="admin-action-grid">
      {['confirmed', 'declined', 'cancelled', 'completed', 'no_show'].map((status) => <button key={status} className="btn" onClick={() => call(() => setAppointmentStatus(appointment.id, status))}>{status}</button>)}
    </div>
...
export default function AdminPage() {
  ...
  return <main className="admin-wrap"><h1>Nails by Brittney Admin</h1>
    ...
    <section><h2>Blocked Times</h2><button className="btn" onClick={async () => {
      ...
      refreshBookingAdmin();
    }}>Add Block</button>{blockedTimes.map((b) => <div key={b.id}>{new Date(b.start_at).toLocaleString()} - {new Date(b.end_at).toLocaleString()} ({b.reason}) <button onClick={async () => { await deleteBlockedTime(b.id); refreshBookingAdmin(); }}>Delete</button></div>)}</section>

    <section><h2>Customers</h2>{customers.map((c) => <article key={c.id} className="card"><h4>{c.first_name} {c.last_name}</h4><p>{c.phone} • {c.email} • pref: {c.communication_preference || 'both'}</p><p>Card: {c.card_on_file_status || 'missing'} {c.card_brand ? `(${c.card_brand} ••••${c.card_last4 || ''})` : ''}</p><ul>{(c.customer_notes || []).map((n) => <li key={n.id}>{new Date(n.created_at).toLocaleDateString()} - {n.note_text}</li>)}</ul></article>)}</section>
