      ...
      <main>
        ...
        <section id="about" className="section">
          <div className="container split">
            ...
            <div>
              <h2>Brittney Prosser, Certified Nail Technician</h2>
              <p>{SAMPLE_BIO}</p>
            </div>
          </div>
        </section>

        <section id="examples" className="section alt">
          <div className="container">
            <h2>Examples</h2>
            <h3>Testimonials</h3>
            <Carousel items={testimonials} renderItem={(item) => <blockquote>&quot;{item.quote}&quot; -{item.customer}</blockquote>} />
            <h3>Gallery</h3>
            <Carousel
              items={gallery}
              renderItem={(item) => (
            ...
            <div className="service-grid">
              {services.map((service) => (
                <article key={service.id} className="card">
                  <NailIcon />
                  <h3>{service.name}</h3>
                  <p className="meta">{service.price_text} • {service.duration}</p>
        ...
        <section id="contact" className="section alt">
          <div className="container">
            <h2>Contact</h2>
            <p><strong>Nails by Brittney: {PHONE_DISPLAY} (call or text)</strong><br />{EMAIL}</p>
            <div className="cta-row">
              <a href={`sms:${PHONE_LINK}`} className="btn primary">Text to Book Now</a>
              <a href={`tel:${PHONE_LINK}`} className="btn">Call to Book Now</a>
              <a href={INSTAGRAM_URL} target="_blank" rel="noreferrer" className="btn ghost">Instagram</a>
